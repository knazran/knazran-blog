<template>
  <div class="flex font-karla">
    <div class="w-full md:w-3/5 flex flex-col space-y-4 pr-6 lg:pr-12">
      <h1 class="text-6xl tracking-wide font-bold text-primary-700">Hi, I'm Nazran</h1>
      <p class="text-xl text-primary-500">
        I build data products and web apps to solve
        problems that matters. A serial hackathoner with over 20 participations in the past 4 years.
      </p>
      <p class="text-xl text-primary-500">Currently based in Kuala Lumpur 🇲🇾</p>
    </div>
    <div class="w-full px-4 md:w-2/5">
      <img class="rounded-lg" src="~/assets/img/portrait.jpg" />
    </div>
  </div>
</template>